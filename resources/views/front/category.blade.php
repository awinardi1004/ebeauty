...
<x-search-nav/>
 ...
 <section class="bg-gray-200 p-4 mt-8">
        <div class="flex justify-between items-center mb-4">
            <h2 class="text-3xl font-semibold text-black">Result</h2>
        </div>
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 lg:grid-cols-6 gap-4">

            @foreach ( $product_categories as $product_category )
            @php
                $variant = $product_category->productVariants->first();
                $now = now();
            ...
            @endphp
                ...
                <div class="relative group bg-red-300 rounded-md p-2 flex flex-col items-center">
                    <div class="w-full aspect-square overflow-hidden rounded-md mb-2">
                        <img src="{{ asset('storage/' . $product_category->productImages->first()->image_path) }}" alt="Foto Produk" class="w-full h-full object-cover" />
                    </div>
                    ...
                    <a href="{{ route('front.details', $product_category) }}"
